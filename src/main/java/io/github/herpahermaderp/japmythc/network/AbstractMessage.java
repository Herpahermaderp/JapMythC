...
public abstract class AbstractMessage<T extends AbstractMessage<T>> implements IMessage, IMessageHandler<T, IMessage> {
	...
	public IMessage onMessage(T message, MessageContext ctx) {
		...
		if(!message.isValidOnSide(ctx.side)) {
			
			throw new RuntimeException("Invalid side " + ctx.side.name() + " for " + message.getClass().getSimpleName());
		}
		
		message.process(JapMythC.proxy.getPlayerEntity(ctx), ctx.side);
		return null;
	}
